...
class MyAdapter(val datas:MutableList<String>) : RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {//데이터와 뷰 홀더를 연결시키는 작업을 함
        ...
        binding.itemTv.text = datas[position] //바인딩에 있는 뷰 객체 중 item_라사이클.xml id를 통해 선언한 객체 데이터 정보 넣기, position


    }
}
//리사이클 순서3 : 데코레이션
      //데코레이션을 위한 클래스, 오버라이드 3개
class MyDecoration(val context: Context) : RecyclerView.ItemDecoration() {
    ...
    //순서3-2 미리 그리기 ondraw
        ...
        //따라서 배경색 먼저 칠하는 느낌 ,캠퍼스에 드로우하기-비트맵 함수
...
class Fragment1 : Fragment() {
    ...
    override fun onCreateView(
        ...
        //프래그먼트1.xml의 뷰 객체를 가져올 수 있는 viewBinding -------// 리사이클 순서1
